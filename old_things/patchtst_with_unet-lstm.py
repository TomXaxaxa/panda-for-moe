...
class PatchTSTPredictionHead(nn.Module):
    ...
    def forward(self, embedding: torch.Tensor):
        ...
        """
        # 获取原始维度
        bs, num_channels, num_patches, d_model = embedding.shape

        # 1. 变形以适应LSTM输入: (B, C, N, D) -> (B*C, N, D)
        # N是序列长度 (num_patches), D是特征维度 (d_model)
        lstm_input = embedding.view(bs * num_channels, num_patches, d_model)

        # 2. 通过LSTM层，得到最后一个时间步的隐藏状态
        # h_n shape: (num_layers, B*C, hidden_size) -> (1, B*C, D)
        # 我们只需要h_n，所以用_忽略其他输出
        _, (h_n, _) = self.lstm_aggregator(lstm_input)
        
        # 3. 提取最后一个时间步的隐藏状态并调整形状
        # 去掉代表层数的维度: (1, B*C, D) -> (B*C, D)
        aggregated_embedding = h_n.squeeze(0)

        # 4. 恢复 num_channels 维度: (B*C, D) -> (B, C, D)
        aggregated_embedding = aggregated_embedding.view(bs, num_channels, d_model)

        # 5. Dropout 和 Projection
        aggregated_embedding = self.dropout(aggregated_embedding)
        
        # output: [bs x num_channels x forecast_len] or tuple
        output = self.projection(aggregated_embedding)
