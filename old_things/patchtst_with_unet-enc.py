...
# =================================================================================
#  +++ 新增模块: EncoderOutputAdapter (用于连接Encoder和Head) +++
# =================================================================================
class EncoderOutputAdapter(nn.Module):
    """
    一个简单的适配器层，用于将U-Net Encoder的输出维度
    映射回Prediction Head期望接收的输入维度。
    """
    def __init__(self, input_dim: int, output_dim: int, norm_eps=1e-6):
        super().__init__()
        self.projection = nn.Linear(input_dim, output_dim)
        self.norm = nn.LayerNorm(output_dim, eps=norm_eps)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: [B, C, N, D_in] (Batch, Channels, NumPatches, InputDim)
        x = self.projection(x)
        x = self.norm(x)
        # return: [B, C, N, D_out] (Batch, Channels, NumPatches, OutputDim)
        return x
...
class PatchTSTForPrediction(PatchTSTPreTrainedModel):
    def __init__(self, config: PatchTSTConfig):
        ...
        # self.skip_connections_depths = [2, 2, 2, 0] # Decoder已禁用，此参数不再需要
        ...
        # config.d_model = original_d_model # 恢复d_model
        
        # +++ 新增 +++
        # 2. 计算Encoder输出的维度
        encoder_output_dim = original_d_model * (2 ** (len(self.depths) - 1))
        
        # 3. 初始化适配器层，将Encoder输出维度映射回原始维度
        self.adapter = EncoderOutputAdapter(input_dim=encoder_output_dim, output_dim=original_d_model)
        # ++++++++++++

        # =========== 完整继承 Distribution Output 逻辑 (这部分保持不变) ===========
    ...
    def forward(
        ...
        # 3. UNet Encoder
        # encoder_output 的形状: [B, C, N_final, D_final]
        # skip_connections 不再需要
        ...
        # +++ 新增 +++
        # 4. 通过适配器层调整维度
        # adapted_output 的形状: [B, C, N_final, D_original]
        adapted_output = self.adapter(encoder_output)
        # ++++++++++++
        
        # 5. Prediction Head
        # 将 adapted_output 而非 decoder_output 传入
        y_hat = self.head(adapted_output)
        ...
        return PatchTSTForPredictionOutput(
            ...
            hidden_states=None, # Encoder-only 结构简化了输出
            attentions=None,    # Encoder-only 结构简化了输出
            loc=loc,
            scale=scale,
        )
