...
class AdvancedAnyVariateAttention(nn.Module):
    ...
    data-driven relational bias between variables using a bilinear model.
    """
    def __init__(
        self,
        embed_dim: int,
        ...
        # Main projection layers for content attention
    ...
    def forward(
        ...
        layer_head_mask: Optional[torch.Tensor] = None,
        output_attentions: bool = False,
        linear_attn: bool = False, # <--- 接收这个导致报错的参数
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
    # ====================================================================
        ...
        tgt_len = src_len = total_len

        # 1. 计算标准的内容注意力 (Content Attention)
        query_states = self.q_proj(hidden_states) * self.scaling
        key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
        value_states = self._shape(self.v_proj(hidden_states), -1, bsz)
        ...
        # 3. 将关系偏差加到内容注意力权重上
        attn_weights = attn_weights + relational_bias
        ...
        if attention_mask is not None:
            ...
            attn_weights = attn_weights + expanded_mask.unsqueeze(1).to(attn_weights.device)

        # 5. Softmax 及后续处理
        attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
        
        # 注意: 我们忽略了 linear_attn 参数，因为我们的新设计总是需要softmax
        attn_weights = nn.functional.softmax(attn_weights, dim=-1)
