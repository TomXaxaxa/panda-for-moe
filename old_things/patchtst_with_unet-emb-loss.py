    ...
    def forward(self, hidden_state, skip_connections, output_attentions=None, channel_attention_mask=None, linear_attn=False):
        ...
        for i, stage in enumerate(self.stages):
            ...
            if stage["upsample"] is not None:
                ...
                # 3. 融合
                # 检查序列长度是否匹配，如果不匹配则填充 (PatchExpansion 可能导致长度不一致)
                if hidden_state_upsampled.shape[1] != skip_reshaped.shape[1]:
                    # 通常是上采样后的长度大于 skip connection
...
class PatchTSTForPrediction(PatchTSTPreTrainedModel):
    ...
    def forward(
        ...
        # 4. UNet Decoder - 现在返回一个列表
        decoder_outputs_list = self.decoder(
            encoder_output,
            skip_connections,
            output_attentions=output_attentions,
            channel_attention_mask=channel_attention_mask,
        ...
        # 5. Prediction Head - 接收列表作为输入
        y_hat = self.head(decoder_outputs_list)
        
        loss_val = None
        
        # 1. 首先，无条件计算最终的输出 y_hat_out
        # 无论是否计算损失，模型都需要有输出
        if self.distribution_output:
            # 如果是概率预测，y_hat_out 就是分布的参数
            y_hat_out = y_hat
        else:
            # 如果是点预测，y_hat_out 是经过反归一化的最终预测值
        ...
        if future_values is not None:
            ...
            if self.distribution_output:
                # 注意：这里现在使用已经定义好的 y_hat_out
                distribution = self.distribution_output.distribution(y_hat_out, loc=loc, scale=scale)
                prediction_loss = nll(distribution, future_values)
                prediction_loss = weighted_average(prediction_loss)
            else:
                # 这里也使用已经定义好的 y_hat_out
        ...
        return PatchTSTForPredictionOutput(
            loss=loss_val,
            prediction_outputs=y_hat_out, # <--- 现在 y_hat_out 总是被定义了
            hidden_states=None, 
            attentions=None,
            loc=loc,
            scale=scale,
