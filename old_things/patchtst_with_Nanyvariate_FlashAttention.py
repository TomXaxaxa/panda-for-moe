...
class VariateFeatureExtractor(nn.Module):
    """
    Extracts a feature vector for each variate using a bank of dilated 1D convolutions.
    This captures multi-scale temporal patterns for each channel.
    """
    def __init__(self, d_model: int, kernel_size: int = 3):
        super().__init__()
        # 根据要求，使用4种不同的空洞率
        self.dilations = [1, 2, 4, 8] 
        self.num_kernels = len(self.dilations)
    ...
    def forward(self, hidden_states: torch.Tensor, num_channels: int, sequence_length: int) -> torch.Tensor:
        """
        Input hidden_states shape: (bsz, num_channels * sequence_length, d_model)
        Returns a tensor of shape: (bsz, num_channels, d_model)
        """
        bsz = hidden_states.shape[0]
        
        # Reshape for convolution: (bsz, nc, sl, dim) -> (bsz*nc, dim, sl)
        x = hidden_states.view(bsz, num_channels, sequence_length, -1)
        x = x.permute(0, 1, 3, 2).reshape(bsz * num_channels, -1, sequence_length)
        
        # 并行通过不同空洞率的卷积核
        conv_outputs = []
        for conv in self.conv_layers:
            # (bsz*nc, dim, sl) -> (bsz*nc, dim) after mean pooling
            conv_out = conv(x)
            pooled_out = conv_out.mean(dim=-1) # 在序列维度上MeanPooling
            conv_outputs.append(pooled_out)
        
        # Stack features from all kernels: (bsz*nc, num_kernels, d_model)
        stacked_features = torch.stack(conv_outputs, dim=1)
        
        # Max-pooling across kernels to get the most salient features
        # (bsz*nc, num_kernels, d_model) -> (bsz*nc, d_model)
        max_pooled_features, _ = torch.max(stacked_features, dim=1)
        
        # Reshape to per-variate features: (bsz, num_channels, d_model)
        variate_features = max_pooled_features.view(bsz, num_channels, -1)
        
        return variate_features
...
class AdvancedAnyVariateAttention(nn.Module):
    """
    An advanced version of AnyVariateAttention that computes a dynamic,
    data-driven relational bias and leverages PyTorch's SDPA backend.
    (RoPE application order corrected)
    """
    def __init__(
        self,
        embed_dim: int,
        ...
        # Dynamic relational modeling components
        self.feature_extractor = VariateFeatureExtractor(d_model=embed_dim)
        self.rel_dim = self.head_dim // 2
        self.fq_linear = nn.Linear(embed_dim, self.num_heads * self.rel_dim)
        self.fk_linear = nn.Linear(embed_dim, self.num_heads * self.rel_dim)
        self.W_rel = nn.Parameter(torch.randn(1, self.num_heads, self.rel_dim, self.rel_dim))
    ...
    def forward(
        ...
        if output_attentions:
            import warnings
            warnings.warn("`output_attentions=True` is not supported with the Flash Attention backend.")

        bsz, total_len, _ = hidden_states.size()

        # 1. Q, K, V 投影, 形状: (bsz, total_len, embed_dim)
        query_states = self.q_proj(hidden_states)
        key_states = self.k_proj(hidden_states)
        value_states = self.v_proj(hidden_states)

        # ==================== 修正RoPE应用顺序 START ====================
        # 2. 先应用 RoPE (在完整的 embed_dim 上)
        if self.use_rope:
            # Reshape for RoPE function: (bsz * total_len, embed_dim)
            query_2d = query_states.view(-1, self.embed_dim)
            key_2d = key_states.view(-1, self.embed_dim)
            
            position_ids = self.get_seq_pos(total_len, key_states.device, key_states.dtype)
            
            # RoPE function expects position_ids to match the first dimension if input is 2D
            # We need to expand it.
            position_ids = position_ids.repeat(bsz)

            key_2d_rope, query_2d_rope = apply_p_rope_to_qk(
                key_2d, query_2d, position_ids, self.embed_dim, self.max_wavelength, self.rope_percent
            )
            
            # Reshape back to 3D
            query_states = query_2d_rope.view(bsz, total_len, self.embed_dim)
            key_states = key_2d_rope.view(bsz, total_len, self.embed_dim)

        # 3. 再拆分到多头 (Multi-head Split)
        # (bsz, total_len, embed_dim) -> (bsz, num_heads, total_len, head_dim)
        query_states = query_states.view(bsz, total_len, self.num_heads, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, total_len, self.num_heads, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, total_len, self.num_heads, self.head_dim).transpose(1, 2)
        # ==================== 修正RoPE应用顺序 END ======================

        # 4. 计算动态关系偏差 (逻辑不变)
        variate_features = self.feature_extractor(hidden_states, num_channels, sequence_length)
        q_rel = self.fq_linear(variate_features).view(bsz, num_channels, self.num_heads, self.rel_dim)
        k_rel = self.fk_linear(variate_features).view(bsz, num_channels, self.num_heads, self.rel_dim)
        q_rel = q_rel.permute(0, 2, 1, 3)
        k_rel = k_rel.permute(0, 2, 1, 3)
        q_rel_w = torch.matmul(q_rel, self.W_rel)
        relational_scores = torch.matmul(q_rel_w, k_rel.transpose(-2, -1))
        
        relational_bias = torch.repeat_interleave(relational_scores, sequence_length, dim=2)
        relational_bias = torch.repeat_interleave(relational_bias, sequence_length, dim=3)
        final_attn_mask = relational_bias

        # 5. 合并外部掩码
        if attention_mask is not None:
            expanded_mask = attention_mask.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, -1, sequence_length, sequence_length)
            expanded_mask = expanded_mask.permute(0, 1, 3, 2, 4).reshape(bsz, total_len, total_len)
            final_attn_mask = final_attn_mask + expanded_mask.unsqueeze(1).to(final_attn_mask.device)

        # 6. 调用 Scaled Dot Product Attention
        ...
        attn_output = attn_output.transpose(1, 2).contiguous().view(bsz, total_len, self.embed_dim)
        attn_output = self.out_proj(attn_output)
...
class PatchTSTEncoderLayerWithRope(nn.Module):
    ...
    def __init__(self, config: PatchTSTConfig):
        ...
        self.self_attn = AdvancedAnyVariateAttention(
